...
\begin{figure}[htp!]
    ...
    \caption{Top-level architecture block diagram \cite{ficDatasheet}}
...
The time, energy, and trigger pulses from each channel are passed to a Front-End Readout block (\emph{FERO}), which generates a digital representation of the rising edge, the \emph{ToA}, with a $\sim$\SI{25}{\pico\second} time bin, and the falling edge, the \emph{ToT}, with a $\sim$\SI{391}{\pico\second} time bin. Both of the timebases for the bins are derived from the reference clock. From here comes the need for a low-jitter clock input.\cite{ficDatasheet}
...
In the final step, an \emph{Aurora} serializer converts the packets from the \emph{FIFO} into an \emph{Aurora 64B/66B} serial stream and transmits them over the high-speed \emph{SLVS} output lines. Statistical and a so called "coarse counter" information is also transmitted if enabled. The stream can be configured to operate at speeds ranging from \SI{80}{\mega\bit\per\second} to \SI{1.28}{\giga\bit\per\second}, in compliance with the \emph{Aurora} specification. \cite{ficDatasheet}\cite{auroraSpec}
